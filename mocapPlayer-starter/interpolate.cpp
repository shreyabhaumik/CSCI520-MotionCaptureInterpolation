...
int main(int argc, char **argv) 
{
  if ( argc != 7 )
  {
    ...
    printf("    q: slerp quaternions\n");
    printf("    Q: slerp+lerp quaternions\n");
  ...
  if (angleRepresentationString[0] == 'e')
    ...
    angleRepresentation = SLERPQUATERNION;
  else if (angleRepresentationString[0] == 'Q')
    angleRepresentation = SLERPLERPQUATERNION;
  else
  {
  ...
  std::string st;
  if (angleRepresentation == EULER)
    st = "EULER";
  else if (angleRepresentation == SLERPQUATERNION)
    st = "SLERPQUATERNION";
  else
    st = "SLERPLERPQUATERNION";

...
#ifdef __APPLE__
#define GL_SILENCE_DEPRECATION  /* Defining GL_SILENCE_DEPRECATION to avoid deprecation messages */
#endif
...
#include <FL/fl_ask.H>
#include <FL/Fl_Text_Display.H> // added to display time
...
// to show ground pattern - concentric squares
SwitchStatus flyCamera = OFF;
int flyCount = 0;
float flyScale = 0.05f, flyRot = 2.0f;
...
void RenderGroundPlane(double groundPlaneSize, double groundPlaneHeight, double rPlane, double gPlane, double bPlane, double ambientFskeleton, double diffuseFskeleton, double specularFskeleton, double shininess)
{
  ...
  // To print concentric squares on the ground
  int n = planeResolution/2, flag = 1;
  float factor[planeResolution][planeResolution], k;
  ...
  for(int i=0; i<planeResolution; i++)
    for(int j=0; j<planeResolution; j++)
    {
      ...
      // float factor = (((i+j) % 2) == 0) ? 0.5f : 1.0f; // this was the line needed for checkerboard pattern
      planeAmbientAct[0] *= factor[i][j];
      planeAmbientAct[1] *= factor[i][j];
      planeAmbientAct[2] *= factor[i][j];
      planeAmbientAct[3] *= factor[i][j];
...
void flyCamera_callback(Fl_Button *obj, long val) // to show ground pattern - concentric squares
{
  fl_message_title("Fly Camera");
  fl_message("This is to show the PATTERN on the GROUND:\nconcentric squares\n");
  flyCamera = ON;
  glwindow->redraw();
}
...
void idle(void*)
{
  ...
  if(playButton == ON) 
  {
    if (saveScreenToFile == SAVE_CONTINUOUS)
    {
      ...
      saveScreenshot(640 * 2, 480 * 2, saveScreenToFileContinuousFilename); // Changed screenshot dimension because of problem in macOS Catalina
  ...
  if (flyCamera == ON)  // to show ground pattern - concentric squares
  {
    if(flyCount<50)
      glScaled(camera.zoom-flyScale, camera.zoom-flyScale, camera.zoom-flyScale);
    else if(flyCount<230)
    {
      glRotated(flyRot, 0.0, 1.0, 0.0);
      glRotated(flyRot, 1.0, 0.0, 0.0);
    }
    else if(flyCount<410)
    {
      glRotated(-flyRot-0.0075, 1.0, 0.0, 0.0);
      glRotated(-flyRot+0.0075, 0.0, 1.0, 0.0);
    }
    else if(flyCount<460)
      glScaled(camera.zoom+flyScale+0.0017, camera.zoom+flyScale+0.0017, camera.zoom+flyScale+0.0017);
    else
    {
      flyCamera = OFF;
      flyCamera_button->box(FL_UP_BOX);
      glwindow->redraw();
      flyCount = 0;
    }
    flyCount++;
...
void GraphicsInit() 
{
  ...
  // 0.0,200.0,r0.81,g0.81,b0.55,a0.1,d0.9,s0.1,sh120.0   values set by starter code
  double groundPlaneHeight = 0.0;
  double groundPlaneSize = 250.0;
  double groundPlaneR = 0.5;
  double groundPlaneG = 0.5;
  double groundPlaneB = 0.65;
  double groundPlaneAmbient = 0.1;
  double groundPlaneDiffuse = 0.8;
  double groundPlaneSpecular = 0.3;
  double groundPlaneShininess = 100.0;
